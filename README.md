# My Photonic Component Library (Adiabatic Components)

This repository provides a showcase of the photonic components I designed throughout my Ph.D. These components can be categorized as power splitters, fiber-to-chip light couplers, mode converters, polarization rotators, and Si-to-SiN light couplers. The GDS files of these devices are given, along with experimental results for most of these components.


## Power Splitters
### 1x2 Power Splitters (Y-Junction Power Splitters)
Here I present my 1x2 power splitter designs and their experimental results. I have designed 2 power splitters, one for an e-beam lithography based fabrication run (for Advanced Micro Foundry, Alberta, Canada) with 120 nm minimum feature size, another for a DUV lithography with CMOS foundry (AMF, Singapore) with 180 nm minimum feature size.

Highlights of the 1x2 Y-Junction power splitters: 
- Low losses
- Broadband
- Polarization insensitivity
- Large minimum feature sizes
- Strong fabrication tolerance
- Much shorter than their adiabatic counterparts!

#### The 1x2 Y-Junction power splitter designed for the ANT foundry process:
[Can Ozcan, Mo Mojahedi, and J. Stewart Aitchison, "*Short, broadband, and polarization-insensitive adiabatic Y-junction power splitters*", Optics Letters, Vol 48, Issue 18, 2023.](https://opg.optica.org/ol/fulltext.cfm?uri=ol-48-18-4901&id=537942)

<p align="center">
  <img src="https://github.com/can-ozcan/My_photonic_component_library/assets/87956141/d9acaeac-dca2-4fa9-b367-cf42232b5df3" width=70% height=70%>
</p>

&nbsp;

#### The 1x2 Y-Junction power splitter designed for AMF:
(Submitted to Photonics) [Invited] Can Ozcan, J. Stewart Aitchison, and Mo Mojahedi, "*Foundry-processed Compact and Broadband Adiabatic Optical Power Splitters with Strong Fabrication Tolerance*".

<p align="center">
  <img src="https://github.com/can-ozcan/My_photonic_component_library/assets/87956141/1ef49e49-2752-4042-bb3b-96fa689c5d79" width=70% height=70%>
</p>

&nbsp;

The performance metrics of the 1x2 Y-Junction power splitters are summarized in the Table below:
<p align="center">
  <img src="https://github.com/can-ozcan/My_photonic_component_library/assets/87956141/f0ac77d7-a788-4efb-a660-68bbf2ab16f6" width=70% height=70%>
</p>

### 2x2 3 dB Power Splitters on 90 nm Slab
This 

<p align="center">
  <img src="https://github.com/can-ozcan/My_photonic_component_library/assets/87956141/a72ed9e3-2e0d-4373-95b0-47ac6a802dff" width=50% height=50%>
</p>

&nbsp;

## TM<sub>0</sub> to TE<sub>1</sub> Mode Converters
[[Invited] Can Ozcan, Mo Mojahedi, and J. Stewart Aitchison, "*Waveguide Taper Profile Optimization for Compact On-chip Adiabatic Devices*", Photonics North Conference, 2023](https://ieeexplore.ieee.org/abstract/document/10222971)



